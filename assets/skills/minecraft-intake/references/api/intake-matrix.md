...
> 核对日期：2026-09（26.3 = 当前正式版，2026-09-15 发布；Paper 26.3 仅 alpha / NeoForge 26.3 仅 beta / CIT 前端最高 26.2）。本表是 minecraft-intake 的唯一事实表；改表必须同步 SKILL.md 的场景速查。
...
| 8 | 26.x 现代线 | 明确 **26.3（当前正式版，2026-09-15）** / 26.2 / 1.21.x（生态差异大：26.3 Paper 只有 alpha、NeoForge 只有 beta、CIT 前端最高 26.2、无 yarn）；客户端侧需求（渲染/CIT） | 1.21.x（26.x 生态仍在跟进；若必须在 26.x，26.2 比 26.3 稳） | 用户已给全 |
| 9 | 资源包（pack 任务，v0.7） | 目标 MC 版本；**包类型**（模型 / CIT / 纹理 / GUI / 字体 / 语言 / 音频 / 全功能）；**客户端前端**（OptiFine / citresewn fork / 纯原版——决定条件通道时代）；是否分版（1.7.10~26.3 多版本）；服务端侧（MMOItems 等自定义物品、是否需要服务端改名） | 按用户实际客户端；**CIT 类包的前端上限是 26.2**（Modrinth 实测 cit-resewn-continuation / cit-resewn-fork 最高 26.2），纯原版包才可 26.3 | 用户已给全 |
...
- **26.3 不要默认**：26.3 是当前正式版（2026-09-15 发布），但 Paper 26.3 只有 `-alpha` 构建、NeoForge 26.3 只有 `-beta` 构建、CIT 前端最高只到 26.2（2026-09 对 repo.papermc.io / maven.neoforged.net / Modrinth 实测）——26.x 内要稳就用 26.2，跨线要稳仍是 1.21.x。
...
1. **客户端前端**：OptiFine / citresewn（continuation fork）/ 纯原版——26.2 时代只有 citresewn fork 的 CIT 通道可用（lore 匹配已死，components 通道）；**26.3 上没有任何 CIT 前端可用**（Modrinth 实测 cit-resewn-continuation / cit-resewn-fork 最高 26.2，2026-09），所以 26.3 的 CIT 类需求不成立、只能降到 26.2；纯原版客户端则 CIT 类需求直接不成立。
...
其他沿用通用规则：目标 MC 版本必问（决定 pack_format 与格式断代——**具体 pack_format 数值矩阵以 minecraft-pack-core 技能的 references 为准，本表不重复维护**，避免两处漂移）、分版需求（1.7.10~26.3 全时代还是单版本）、pack.config.json 的 names/syncGroups 是否需要预填。详情交叉引用：minecraft-pack-core（结构/矩阵）、minecraft-pack-cit（条件通道）、minecraft-pack-lang（服务端边界）。
