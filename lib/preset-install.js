...
/** The default preset id shipped by this package (the directory name under the root). */
...
export function resolveDshHome(env = process.env) {
  ...
  const isValid = fromEnv !== undefined && fromEnv.trim().length > 0 && !fromEnv.includes('..')
  const selected = isValid ? fromEnv : join(homedir(), '.dsh')
  return resolve(expandTilde(selected))
}

/**
 * Install one bundled agent preset into the harness-home preset root unless a
 * composition already exists there.
 * @param {object} [options]
 * @param {string} options.srcDir - package preset directory holding the files to install.
 * @param {string} [options.presetId] - directory name under the preset root; defaults to {@link PRESET_ID}.
...
export async function ensurePresetInstalled(options = {}) {
  const { srcDir, presetId = PRESET_ID, home = resolveDshHome(), log = () => {}, debug = () => {} } = options
  if (!srcDir) throw new Error('ensurePresetInstalled: srcDir is required')
  const destDir = join(resolve(home), USER_PRESET_DIR, presetId)
  ...
  if (compositionPresent) {
    debug(`agent preset '${presetId}' already installed at ${destDir} — skipping (remove the directory to force a refresh)`)
  ...
  log(`installed agent preset '${presetId}' to ${destDir} (${written.join(', ')}) — new sessions can pick it from the preset selector`)
