...
export function gradleResultView(args, result) {
  return terminalResultView(result)
}

/**
 * Shared completed-card view for any tool whose rendered text carries the
 * dsh exit/signal marker block (mc_gradle, mc_codex).
 * @param {{ content: Array<{ type: string, text?: string }>, isError?: boolean }} result
 * @returns {{ card: 'terminal', output: string, exitCode?: number, signal?: string }
 *   | { card: 'generic', content: Array<{ type: 'text', text: string }> } | undefined}
 */
export function terminalResultView(result) {
...
/**
 * Pending-card view for mc_codex: a terminal card headed by the Codex
 * invocation, with the working directory as the card cwd — the same shape
 * gradleCallView uses, so both tools read as "a command is running here".
 * @param {{ mode?: string, sandbox?: string, description?: string, projectDir?: string }} args
 * @returns {{ card: 'terminal', title: string, description?: string, cwd?: string }}
 */
export function codexCallView(args) {
  const mode = args.mode ?? 'app-server'
  return {
    card: 'terminal',
    title: `codex ${mode} (${args.sandbox ?? 'workspace-write'})`,
    ...(args.description !== undefined ? { description: args.description } : {}),
    ...(args.projectDir !== undefined ? { cwd: args.projectDir } : {}),
  }
}
...
/**
 * Completed-card view for mc_codex: identical marker handling to mc_gradle, so
 * a failed Codex run shows an exit pill instead of grey body text.
 * @param {object} args - the call args (unused; kept for signature parity).
 * @param {{ content: Array<{ type: string, text?: string }>, isError?: boolean }} result
 * @returns {{ card: 'terminal', output: string, exitCode?: number, signal?: string }
 *   | { card: 'generic', content: Array<{ type: 'text', text: string }> } | undefined}
 */
export function codexResultView(args, result) {
  return terminalResultView(result)
}
