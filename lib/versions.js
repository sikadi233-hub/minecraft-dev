...
export const PLATFORM_LINES = {
  paper: [
    ...
    // 26.3 (2026-09-15) keeps Java 25. Paper has no -stable builds for it yet
    // (26.3.build.NN-alpha only, verified against repo.papermc.io 2026-09), so
    // the coordinate is the same `26.3.build.+` dynamic range as 26.2's.
    { line: '26.3', javaVersion: 25, defaultMcVersion: '26.3', templateDir: 'paper' },
  ],
  fabric: [
    ...
    // 26.3: loader 0.19.5 / Fabric API 0.161.0+26.3 from meta.fabricmc.net and
    // maven.fabricmc.net (2026-09). loom stays on the 1.17 line on purpose:
    // 1.18.2 requires the *Gradle JVM* to be Java 25 ("Dependency requires at
    // least JVM runtime version 25. This build uses a Java 22 JVM"), which would
    // force every user to launch Gradle on JDK 25; 1.17.21 builds 26.3 fine on
    // an older JVM. Verified by a real build (2026-09).
    { line: '26.3', javaVersion: 25, defaultMcVersion: '26.3', templateDir: 'fabric-26',
      coords: { gradleVersion: '9.5.1', loomVersion: '1.17.21', loaderVersion: '0.19.5', fabricApiVersion: '0.161.0+26.3' } },
  ],
  forge: [
  ...
  neoforge: [
    ...
    // 1.21.1 needs an explicit modLoader/loaderVersion: FML only made javafml the
    // default on later 21.x, so the 21.11 template (no modLoader) fails to load there
    // with "Missing ModLoader". NeoForge 21.1.249 + moddev 2.0.144 + Gradle 9.2.1
    // verified by a real client install build (2026-09).
    { line: '1.21.1', javaVersion: 21, defaultMcVersion: '1.21.1', templateDir: 'neoforge-1.21.1',
      coords: { gradleVersion: '9.2.1', moddevPlugin: 'net.neoforged.moddev', moddevVersion: '2.0.144', neoVersion: '21.1.249' } },
    ...
    // 26.3 ships NeoForge builds only as -beta so far (26.3.0.12-beta latest on
    // maven.neoforged.net, 2026-09); moddev 2.0.147 is the current plugin.
    { line: '26.3', javaVersion: 25, defaultMcVersion: '26.3', templateDir: 'neoforge-26',
      coords: { gradleVersion: '9.2.1', moddevPlugin: 'net.neoforged.moddev', moddevVersion: '2.0.147', neoVersion: '26.3.0.12-beta' } },
  ],
  spigot: [
...
/**
 * Platforms whose coordinates are pinned to exactly one Minecraft version per
 * line (the mod loaders). Substituting a different full version there silently
 * emits a project for the wrong MC — e.g. 1.21.1 resolved to the 1.21.11
 * template, which carries no `modLoader` and fails to load with
 * "Missing ModLoader" — so those requests fail loudly instead. Paper/Spigot stay
 * lenient: a specific version there still drives paperApiVersion.
 */
const STRICT_PINNED_PLATFORMS = new Set(['fabric', 'forge', 'neoforge'])
...
export function resolveLine(platform, minecraftVersion) {
  ...
  const requested = minecraftVersion.replace(/\.x$/i, '')
  // 1) Exact pinned version first: lines can share a prefix (1.21 vs 1.21.1), and
  //    a plain prefix match would hand 1.21.1 the 1.21.11 entry.
  const exact = entries.find(e => e.defaultMcVersion === requested)
  if (exact) return exact
  const line = normalizeMcLine(requested)
  const entry = entries.find(e => line === e.line) ?? entries.find(e => line.startsWith(e.line))
  if (!entry) {
  ...
  // 2) A full version no template pins must never be silently substituted: on lines
  //    whose coordinates are version-locked (mod platforms) that produces a project
  //    for a different Minecraft version. Fail loudly with the pinned versions.
  const isFullVersion = requested.split('.').length >= 3
  if (isFullVersion && STRICT_PINNED_PLATFORMS.has(platform)) {
    const pinned = entries.map(e => e.defaultMcVersion).join(', ')
    throw new Error(`no template pinned to ${platform} ${minecraftVersion}; supported Minecraft versions: ${pinned}`)
  }
  // 3) The same hazard for a two-part version that only *prefix*-matched a line:
  //    with 26.2 and 26.3 both pinned, `fabric 26.4` used to fall through to the
  //    `26` entry and silently build for 26.2. A strict platform now only accepts
  //    a request that names a pinned version or hits a line entry exactly.
  if (STRICT_PINNED_PLATFORMS.has(platform) && line !== entry.line && entry.defaultMcVersion !== requested) {
    const pinned = entries.map(e => e.defaultMcVersion).join(', ')
    throw new Error(`no template for ${platform} ${minecraftVersion}; supported Minecraft versions: ${pinned}`)
  }
  return entry
}
