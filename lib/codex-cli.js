...
/**
 * Locate the rollout file a `codex exec` session left behind.
 *
 * Every non-ephemeral Codex run is persisted under
 * `~/.codex/sessions/<yyyy>/<mm>/<dd>/rollout-<stamp>-<sessionId>.jsonl`. The
 * desktop app's conversation list does NOT surface these `source: "exec"`
 * threads, so reporting the file path is how a user actually finds and reads
 * what Codex did. Bounded walk; a miss is `null`, never a throw.
 * @param {string} sessionId - the session id extracted from the run output.
 * @param {{ sessionsRoot?: string, home?: string, maxDepth?: number, maxVisited?: number }} [options]
 * @returns {Promise<string|null>} absolute rollout path, or null when not found.
 */
export async function findRolloutPath(sessionId, options = {}) {
  ...
  async function walk(dir, depth) {
    ...
    // Newest date directory first, so a fresh run is found quickly.
    const sorted = [...entries].sort((a, b) => String(b.name).localeCompare(String(a.name)))
    for (const entry of sorted) {
      ...
      if (entry.isFile() && entry.name.includes(sessionId)) return full
    }
    return null
  }
 ...
 * ran, the process output, what changed, and how to reach the Codex thread
 * again. The marker block stays last so terminal cards keep their exit pill.
...
export function renderCodexResult(result, options = {}) {
  ...
  if (result.sessionId) {
    footer.push(`session: ${result.sessionId} — continue it with: codex exec resume ${result.sessionId} "…"`)
  }
  if (result.rolloutPath) {
    // The desktop app lists only its own threads; an `exec` run is reachable
    // through this file and through `codex exec resume <id>`.
    footer.push(`rollout: ${result.rolloutPath} (not listed in the Codex app sidebar)`)
  }
...
export async function runCodex(options) {
  ...
  const sessionId = extractSessionId(result.output.text)
  const rolloutPath = sessionId === null
    ? null
    : await findRolloutPath(sessionId, {
      home: internals.discovery?.home ?? homedir(),
      sessionsRoot: internals.rolloutRoot,
    })
